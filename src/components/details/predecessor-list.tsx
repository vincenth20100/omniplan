                ...
                <Popover open={addPopoverOpen} onOpenChange={setAddPopoverOpen}>
                    ...
                    <PopoverContent className="w-[300px] sm:w-[400px] p-0" onOpenAutoFocus={(e) => e.preventDefault()}>
                         <RelationshipComboboxContent
                            ...
                            excludedTaskIds={predecessorLinks.map(l => l.source)}
                            onSelectTask={(newSourceId) => {
                                dispatch({
                                    type: 'ADD_LINK',
                                    payload: {
                                        source: newSourceId,
                                ...
                                <TableCell className={cellClass} title={sourceTask?.name}>
                                    <Popover open={editingLinkId === link.id} onOpenChange={(open) => !open && setEditingLinkId(null)}>
                                        ...
                                        <PopoverContent className="w-[300px] sm:w-[400px] p-0" onOpenAutoFocus={(e) => e.preventDefault()}>
                                                ...
                                                allTasks={allTasks}
                                                currentTaskId={currentTaskId}
                                                excludedTaskIds={predecessorLinks.map(l => l.source).filter(id => id !== link.source)}
                                                onSelectTask={(newSourceId) => {
                                                    dispatch({ type: 'UPDATE_LINK', payload: { id: link.id, source: newSourceId } });
