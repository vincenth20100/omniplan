...
class CalendarService {
  ...
  public isWorkingDay(date: Date, calendar: Calendar): boolean {
    ...
    // ⚡ Bolt: Native date manipulation is much faster than date-fns in hot loops
    ...
    if (calendar.exceptions) {
      for (const ex of calendar.exceptions) {
        if (ex.isActive && ex.start && ex.finish) {
          ...
          if (sTime >= exStart.getTime() && sTime <= exFinish.getTime()) {
            return false;
          }
        }
      }
    ...
    // ⚡ Bolt: Inline ternary is faster than String().padStart() in tight loops
  ...
  public findNextWorkingDay(date: Date, calendar: Calendar, direction: 1 | -1 = 1): Date {
    // ⚡ Bolt: Mutate new Date instead of instantiating new objects via addDays
